#!/usr/bin/env python3
"""
Train Extreme10 selectors — 17-dimensional graph-topology + error-mass feature expansion.

Drop-in replacement for train_extreme9_selectors.py.  All training infrastructure
(band-reward, pointwise, aggregated_selacc10 objectives; cross-validation;
BestofN evaluation) is identical; only the feature set changes:
  - 11 Extreme9 features (dc_z, dc_r, reflection_count_r + 8 local tok_conf)
  +  3 graph topology features (local_cc_r, norm_degree_r, cluster_size_r)
     computed from the 64-run Jaccard distance matrix D (Option C)
  +  3 error-mass features (instability_mass_r, tail_variance_r,
                             event_pre_post_delta_r)

Key difference from Extreme9 training:
  _extract_problem_payloads() also:
    1. Builds RunViews for each sample in the problem group
    2. Calls DistanceEngine.dense_matrix() to get D (1 thread per worker)
    3. Calls extract_graph_topo_raw(D) → payload["graph_raw"]

Threading: 1 DistanceEngine thread per worker process.
Constraint: --workers * 1 <= 16, so --workers 4 (default) is fine.

Models saved to models/ml_selectors/extreme10_{best,worst}.pkl
"""
...
from nad.core.selectors.extreme10_impl import (
    EXTREME10_FEATURE_NAMES,
    LinearRankModel10,
    N_EXTREME10_FEATURES,
    accumulate_extreme10_scores,
    build_extreme10_features,
    extract_extreme10_raw_values,
)
from nad.core.selectors.graph_topo_impl import extract_graph_topo_raw
from nad.core.selectors.trajectory_impl import DEFAULT_REFLECTION_THRESHOLD
from nad.core.views.reader import CacheReader
from nad.core.views.reader import ViewSpec
...
def _extract_problem_payloads(batch: list[dict[str, Any]], reflection_threshold: float) -> list[dict[str, Any]]:
    """
    Extract raw feature values (Extreme10: 14 keys) and graph topology (3 keys)
    for each problem in the batch.

    Graph topology requires computing the full pairwise distance matrix D.
    We use DistanceEngine with 1 thread (worker-level parallelism handles the rest).
    """
    _set_single_thread_env()
    ...
    from nad.core.distance.engine import DistanceEngine, DistanceSpec
    from nad.core.views.spec import Agg, CutSpec, CutType, Order, ViewSpec

    dist_spec = DistanceSpec("ja", num_threads=1)
    ...
    for spec in batch:
        ...
        # ── Extreme10 raw values (14 keys, no D needed) ───────────────────
        raw_values = extract_extreme10_raw_values(
            ctx,
            reflection_threshold=float(reflection_threshold),
        )

        # ── Distance matrix for graph topology ────────────────────────────
        try:
            from nad.core.views.spec import Agg, CutSpec, CutType, Order, ViewSpec
            default_vspec = ViewSpec(
                agg=Agg.MAX,
                cut=CutSpec(CutType.MASS, 1.0),
                order=Order.BY_KEY,
            )
            views = [reader.get_run_view(rid, default_vspec) for rid in run_ids]
            engine = DistanceEngine(dist_spec)
            D = engine.dense_matrix(views)
            graph_raw = extract_graph_topo_raw(D)
        except Exception as exc:
            # Fall back to empty / zero graph features if D computation fails
            n = len(run_ids)
            graph_raw = {
                "local_cc": np.zeros(n, dtype=np.float64),
                "norm_degree": np.zeros(n, dtype=np.float64),
                "cluster_size_frac": np.full(n, 1.0 / max(n, 1), dtype=np.float64),
            }

        payload = dict(spec)
        payload["raw_values"] = {
            key: np.asarray(arr, dtype=np.float64)
            for key, arr in raw_values.items()
        }
        payload["graph_raw"] = {
            key: np.asarray(arr, dtype=np.float64)
            for key, arr in graph_raw.items()
        }
...
def _fit_band_reward_model(
    ...
    model = LinearRankModel10(weights=np.asarray(best_weights, dtype=np.float64))
    return model, {
...
def _fit_aggregated_selacc10_model(
    ...
    def _evaluate(candidate: np.ndarray) -> None:
        nonlocal best_pass, best_any
        weights = normalize_weight_direction(candidate)
        key = tuple(np.round(weights, 8).tolist())
        if key in seen:
            return
        seen.add(key)
        best_model = LinearRankModel10(weights=weights)
    ...
    best_model = LinearRankModel10(weights=selected_weights)
    worst_model = ZeroRankModel()
...
def _evaluate_payloads(
    ...
    for payload_index, payload in enumerate(payloads):
        ...
        raw_values = payload["raw_values"]
        graph_raw = payload["graph_raw"]
        score_payload = accumulate_extreme10_scores(
            best_model=best_model,
...
def train_selector_artifacts(
    *,
    ...
    stats = {
        "feature_names": EXTREME10_FEATURE_NAMES,
...
def main():
    ap = argparse.ArgumentParser(description="Train Extreme10 selectors (17-dim graph-topology + error-mass feature expansion)")
    ...
    ap.add_argument("--artifact-prefix", default="extreme10", help="Artifact filename prefix")
    ...
    ap.add_argument("--workers", type=int, default=4, help="Parallel workers for raw-value + distance extraction")
    ...
    print(f"[Extreme10] Collecting raw values + D for {len(train_specs)} training problems")
    train_payloads, train_bucket_sizes = _load_problem_payloads(
        train_specs,
        reflection_threshold=reflection_threshold,
        workers=int(args.workers),
    )
    print(f"[Extreme10] Collecting raw values + D for {len(val_all_specs)} validation problems")
