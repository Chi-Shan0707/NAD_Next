...
def plot_figure(stats: Dict, output_path: Path, layout: str = '1x2', error_type: str = 'none'):
    ...
    if n_models == 1:
        # Single model: 1 row, n_datasets columns
    ...
    for i, model in enumerate(models):
        for j, dataset in enumerate(datasets):
            ...
            for selector in selectors:
                ...
                # Plot line with markers (mean only, no error shading)
            ...
            if all_means:
                ...
                # Add padding (15% of spread, minimum 3%)
                padding = max(data_spread * 0.15, 3)
...
def main():
    parser = argparse.ArgumentParser(description='Plot extended downsample ablation results')
    parser.add_argument('--results-dir', type=str, required=True,
                       help='Path to results directory')
    parser.add_argument('--output', type=str, default=None,
                       help='Output file path (default: results_dir/figures/extended_downsample)')
    ...
    if args.output:
        ...
        output_path = results_dir / 'figures' / 'extended_downsample'
    ...
    # Generate figure (mean only, no error shading)
    print("Generating figure (mean of 10 seeds)...")
    plot_figure(stats, output_path)

    # Also generate figure with CI for comparison
    print("Generating figure with CI...")
    plot_figure_with_ci(stats, output_path.parent / 'extended_downsample_ci')
