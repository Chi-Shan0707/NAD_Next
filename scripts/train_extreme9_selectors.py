#!/usr/bin/env python3
"""
Train Extreme9 selectors — 11-dimensional local-confidence feature expansion.

Drop-in replacement for train_extreme8_selectors.py.  All training infrastructure
...
BestofN evaluation) is identical; only the feature set changes:
  - 3 Extreme8 features (dc_z, dc_r, reflection_count_r)
  + 8 local tok_conf features (tail_2k_r, tail_q10_r, lgc_512_r, lgc_2k_r,
                                bottom_q10_r, head_tail_gap_r,
                                last_event_tail_conf_r, event_nonevent_gap_r)

Models saved to models/ml_selectors/extreme9_{best,worst}.pkl
"""
...
from nad.core.selectors.extreme9_impl import (
    EXTREME9_FEATURE_NAMES,
    LinearRankModel9,
    accumulate_extreme9_scores,
...
def _extract_problem_payloads(batch: list[dict[str, Any]], reflection_threshold: float) -> list[dict[str, Any]]:
    ...
    for spec in batch:
        ...
        raw_values = extract_extreme9_raw_values(
            ctx,
            reflection_threshold=float(reflection_threshold),
        )
        payload = dict(spec)
        # Serialise all raw value arrays for inter-process communication
        payload["raw_values"] = {
...
def _fit_band_reward_model(
    ...
    model = LinearRankModel9(weights=np.asarray(best_weights, dtype=np.float64))
    return model, {
...
def _fit_aggregated_selacc10_model(
    ...
    def _evaluate(candidate: np.ndarray) -> None:
        ...
        if key in seen:
            return
        seen.add(key)
        best_model = LinearRankModel9(weights=weights)
    ...
    best_model = LinearRankModel9(weights=selected_weights)
    worst_model = ZeroRankModel()
...
def _evaluate_payloads(
    ...
    for payload_index, payload in enumerate(payloads):
        ...
        raw_values = payload["raw_values"]
        score_payload = accumulate_extreme9_scores(
            best_model=best_model,
...
def train_selector_artifacts(
    *,
    ...
    stats = {
        "feature_names": EXTREME9_FEATURE_NAMES,
...
def main():
    ap = argparse.ArgumentParser(description="Train Extreme9 selectors (11-dim local-conf feature expansion)")
    ...
    ap.add_argument("--artifact-prefix", default="extreme9", help="Artifact filename prefix")
    ...
    print(f"[Extreme9] Collecting raw values for {len(train_specs)} training problems")
    train_payloads, train_bucket_sizes = _load_problem_payloads(
        train_specs,
        reflection_threshold=reflection_threshold,
        workers=int(args.workers),
    )
    print(f"[Extreme9] Collecting raw values for {len(val_all_specs)} validation problems")
