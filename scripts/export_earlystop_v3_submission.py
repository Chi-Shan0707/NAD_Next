#!/usr/bin/env python3
"""Export Early Stop v3 (neuron-structural coding signal) submission from cache_test."""
...
from nad.ops.earlystop_v3 import (
    CODING_SIGNAL,
    CODING_TOKEN_BLEND,
    GPQA_TRAJ_BLEND,
    get_domain,
    score_cache_entry_earlystop_v3,
)


def main() -> None:
    ap = argparse.ArgumentParser(description="Export Early Stop v3 submission")
    ...
    ap.add_argument("--method-name", default="domain_aware_v3")
    ap.add_argument("--filename", default="domain_aware_v3.json")
    ap.add_argument("--max-problems", type=int, default=None)
    args = ap.parse_args()

    print(f"v3 config: coding_signal={CODING_SIGNAL}, coding_blend={CODING_TOKEN_BLEND}, gpqa_blend={GPQA_TRAJ_BLEND}")
    ...
    for entry in entries:
        ...
        ps = score_cache_entry_earlystop_v3(entry, max_problems=args.max_problems)
