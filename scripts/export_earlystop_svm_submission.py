#!/usr/bin/env python3
"""Export EarlyStop SVM submission from cache_test."""
...
from nad.ops.earlystop_svm import (
    ...
    score_cache_entry_earlystop_svm,
)


def main() -> None:
    ap = argparse.ArgumentParser(description="Export EarlyStop SVM submission")
    ap.add_argument("--cache-root", default="/home/jovyan/public-ro/MUI_HUB/cache_test")
    ap.add_argument("--model-path", default="models/ml_selectors/earlystop_svm_hybrid_v1.pkl")
    ap.add_argument("--out-dir", default="submission/EarlyStop")
    ap.add_argument("--method-name", default="earlystop_svm_hybrid_v1")
    ap.add_argument("--filename", default="earlystop_svm_hybrid_v1.json")
    ...
    bundle = load_earlystop_svm_bundle(model_path)
    ...
    for entry in entries:
        ...
        ps = score_cache_entry_earlystop_svm(entry, bundle, max_problems=args.max_problems)
