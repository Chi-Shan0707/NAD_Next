...
| science | diagonal | 73.34% | 74.11% | 74.52% | -0.77 pts | -1.18 pts |
...
| science | adjacent backward | 72.40% | 73.05% | 73.36% | -0.64 pts | -0.96 pts |
...
| math | 10->100 | 96.38% | 96.58% | 96.68% | -0.20 pts | tie |
| science | 100->40 | 74.61% | 74.68% | 74.37% | -0.07 pts | tie |
| science | 100->70 | 76.03% | 76.11% | 76.04% | -0.09 pts | tie |
...
- **早期 anchor 是否只学到 coarse signal？** `math` 不是：`10→100` 仍只差 `-0.20 pts`，说明早期 basis 已经捕获到大部分稳定决策边界。`science` 更像是 **yes**：`10→100` 掉到 `-5.60 pts`，而 `70→100` 只差 `-1.54 pts`，说明早期 science anchor 更像 coarse / under-formed signal，late anchor 才带有 completion-heavy information。
...
- Matrix CSV: `results/tables/earlystop_ssl/baseline_cross_anchor_transfer_matrix.csv`
- Delta CSV: `results/tables/earlystop_ssl/baseline_cross_anchor_transfer_deltas.csv`
- Summary CSV: `results/tables/earlystop_ssl/baseline_cross_anchor_transfer_summary.csv`
